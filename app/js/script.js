...
requestStack.onload = function () {
    ...
    if (requestStack.status >= 200 && requestStack.status < 400) {
        stackData.forEach(stack => {
            techTable += "<tr><td width='60%'>"+stack.tech_name+"</td><td><input type='checkbox' data-name='"+stack.tech_name+"' data-id='"+stack.id+"' id='tech_"+techRowCounter+"' onclick='setState(this.id)'></td><td><select id='tech_"+techRowCounter+"-exp' disabled>"+options+"</select> yrs</td></tr>";
            techRowCounter++;
        });
    }

    techTable += "<tr><td colspan='3' align='center'><button class='button-9' role='button' onClick='getTalent()'>Search</button></td></tr>";
    techTable += "<tr><td colspan='3' align='center'><button class='button-9' role='button' onClick='resetCards()'>Reset</button></td></tr>";
...
request.onload = function () {
    ...
    if (request.status >= 200 && request.status < 400) {
        ...
        data.forEach(talent => {
            ...
            email.innerHTML = "<a href='mailto:"+talent.email+"'><button class='button-9' role='button'>Contact "+talent.name+"</button></a>";
...
function getTalent() {
    ...
    for(i=1;i<=techRowCounter;i++){
        ...
        if(element.checked==true){
            ...
            if (techExp != "x") {
                ...
                noExp = true
                errorMsg += "<i class='fa fa-exclamation-triangle' style='color:red'/></i>&nbsp;&nbsp;&nbsp;No Exp value for "+dataName+"<br />";
    ...
    if(!noExp && !noTech){
        ...
        if(noTech){
            ...
            errorMessage.innerHTML = "<i class='fa fa-exclamation-triangle' style='color:red'/></i>&nbsp;&nbsp;&nbsp;No Selection Made";
